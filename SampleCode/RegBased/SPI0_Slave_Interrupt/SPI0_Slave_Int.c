...
//***********************************************************************************************************
//  File Function: MUG51 SPI in Slave mode demo code
//***********************************************************************************************************
#include "MUG51_IAR.h"
...
void main(void)
{
    ...
    if(u8Receive_Data[0] != 0x90)               // Receive 1st DATA from master 
        SPI_Error(); 
    if(u8Receive_Data[1] != 0x01)               // Receive 2nd DATA from master 
        SPI_Error();
    if(u8Receive_Data[2] != 0x02)               // Receive 3rd DATA from master 
        SPI_Error();
    if(u8Receive_Data[3] != 0x03)               // Receive 4th DATA from master 
        SPI_Error();
    if(u8Receive_Data[4] != 0x04)               // Receive 5th DATA from master 
        SPI_Error();
